...
L_80075788:
    // lh          $t2, 0x0($a1)
    ctx->r10 = MEM_HS(0X0, ctx->r5);
    // lh          $t5, 0x6($a1)
    ctx->r13 = MEM_HS(0X6, ctx->r5);
    // multu       $t0, $t2
    result = U64(U32(ctx->r8)) * U64(U32(ctx->r10)); lo = S32(result >> 0); hi = S32(result >> 32);
    // lh          $t3, 0x2($a1)
    ctx->r11 = MEM_HS(0X2, ctx->r5);
    // lh          $t6, 0x8($a1)
    ctx->r14 = MEM_HS(0X8, ctx->r5);
    // mflo        $t8
    ctx->r24 = lo;
    // lh          $t4, 0x4($a1)
    ctx->r12 = MEM_HS(0X4, ctx->r5);
    // lh          $t7, 0xA($a1)
    ctx->r15 = MEM_HS(0XA, ctx->r5);
    // multu       $t1, $t5
