...
void sub_8001561C(uint8_t* rdram, recomp_context* ctx) 
{
    ...
    // Ёмулируем вызов sub_8001561C с a1=2 (sector ready)
    ...
    uint8_t state = stream[1];

    if (state == 0) {
        // ѕервый вызов Ч настраиваем и читаем
        *(uint32_t*)(stream + 20) = *(uint32_t*)(stream + 24);
        uint32_t sectors = *(uint32_t*)(stream + 40);
        uint32_t dest = *(uint32_t*)(stream + 32);
        stream[1] = 1; // state = reading

        // „итаем данные сразу
        KFCD_CdlReadN(rdram, ctx);

        // —разу переводим в state 2
        stream[1] = 2;
