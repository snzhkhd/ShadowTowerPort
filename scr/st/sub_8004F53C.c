#include "recomp.h"
#include "disable_warnings.h"

void sub_8004F53C(uint8_t* rdram, recomp_context* ctx) {
    uint64_t hi = 0, lo = 0, result = 0;
    unsigned int rounding_mode = DEFAULT_ROUNDING_MODE;
    int c1cs = 0; 
    // addiu       $sp, $sp, -0x38
    ctx->r29 = ADD32(ctx->r29, -0X38);
    // sw          $s5, 0x24($sp)
    MEM_W(0X24, ctx->r29) = ctx->r21;
    // addu        $s5, $a0, $zero
    ctx->r21 = ADD32(ctx->r4, 0);
    // lui         $v0, 0x801C
    ctx->r2 = S32(0X801C << 16);
    // sw          $s3, 0x1C($sp)
    MEM_W(0X1C, ctx->r29) = ctx->r19;
    // addiu       $s3, $v0, 0x2230
    ctx->r19 = ADD32(ctx->r2, 0X2230);
    // sw          $s4, 0x20($sp)
    MEM_W(0X20, ctx->r29) = ctx->r20;
    // addu        $s4, $zero, $zero
    ctx->r20 = ADD32(0, 0);
    // sw          $s7, 0x2C($sp)
    MEM_W(0X2C, ctx->r29) = ctx->r23;
    // addiu       $s7, $s3, -0x1F20
    ctx->r23 = ADD32(ctx->r19, -0X1F20);
    // sw          $s2, 0x18($sp)
    MEM_W(0X18, ctx->r29) = ctx->r18;
    // addiu       $s2, $s3, 0x12
    ctx->r18 = ADD32(ctx->r19, 0X12);
    // sw          $ra, 0x30($sp)
    MEM_W(0X30, ctx->r29) = ctx->r31;
    // sw          $s6, 0x28($sp)
    MEM_W(0X28, ctx->r29) = ctx->r22;
    // sw          $s1, 0x14($sp)
    MEM_W(0X14, ctx->r29) = ctx->r17;
    // sw          $s0, 0x10($sp)
    MEM_W(0X10, ctx->r29) = ctx->r16;
L_8004F57C:
    // lhu         $v1, -0x8($s2)
    ctx->r3 = MEM_HU(-0X8, ctx->r18);
    // ori         $v0, $zero, 0xFFFF
    ctx->r2 = 0 | 0XFFFF;
    // beq         $v1, $v0, L_8004F6D0
    if (ctx->r3 == ctx->r2) {
        // nop
    
        goto L_8004F6D0;
    }
    // nop

    // lw          $v0, 0x71B0($s7)
    ctx->r2 = MEM_W(0X71B0, ctx->r23);
    // nop

    // beq         $s3, $v0, L_8004F6D0
    if (ctx->r19 == ctx->r2) {
        // addu        $v0, $v1, $zero
        ctx->r2 = ADD32(ctx->r3, 0);
        goto L_8004F6D0;
    }
    // addu        $v0, $v1, $zero
    ctx->r2 = ADD32(ctx->r3, 0);
    // sll         $v1, $v0, 1
    ctx->r3 = S32(ctx->r2) << 1;
    // addu        $v1, $v1, $v0
    ctx->r3 = ADD32(ctx->r3, ctx->r2);
    // sll         $v1, $v1, 3
    ctx->r3 = S32(ctx->r3) << 3;
    // addu        $s6, $v1, $s7
    ctx->r22 = ADD32(ctx->r3, ctx->r23);
    // lhu         $v0, 0x4($s6)
    ctx->r2 = MEM_HU(0X4, ctx->r22);
    // nop

    // beq         $v0, $zero, L_8004F6D0
    if (ctx->r2 == 0) {
        // nop
    
        goto L_8004F6D0;
    }
    // nop

    // lw          $v1, 0x12($s2)
    ctx->r3 = MEM_W(0X12, ctx->r18);
    // lw          $v0, 0x4($s5)
    ctx->r2 = MEM_W(0X4, ctx->r21);
    // nop

    // subu        $s1, $v1, $v0
    ctx->r17 = SUB32(ctx->r3, ctx->r2);
    // addiu       $v0, $s1, 0x2800
    ctx->r2 = ADD32(ctx->r17, 0X2800);
    // sltiu       $v0, $v0, 0x5001
    ctx->r2 = ctx->r2 < 0X5001 ? 1 : 0;
    // beq         $v0, $zero, L_8004F6D0
    if (ctx->r2 == 0) {
        // nop
    
        goto L_8004F6D0;
    }
    // nop

    // lw          $v1, 0x1A($s2)
    ctx->r3 = MEM_W(0X1A, ctx->r18);
    // lw          $v0, 0xC($s5)
    ctx->r2 = MEM_W(0XC, ctx->r21);
    // nop

    // subu        $s0, $v1, $v0
    ctx->r16 = SUB32(ctx->r3, ctx->r2);
    // addiu       $v0, $s0, 0x2800
    ctx->r2 = ADD32(ctx->r16, 0X2800);
    // sltiu       $v0, $v0, 0x5001
    ctx->r2 = ctx->r2 < 0X5001 ? 1 : 0;
    // beq         $v0, $zero, L_8004F6D0
    if (ctx->r2 == 0) {
        // mult        $s1, $s1
        result = S64(S32(ctx->r17)) * S64(S32(ctx->r17)); lo = S32(result >> 0); hi = S32(result >> 32);
        goto L_8004F6D0;
    }
    // mult        $s1, $s1
    result = S64(S32(ctx->r17)) * S64(S32(ctx->r17)); lo = S32(result >> 0); hi = S32(result >> 32);
    // mflo        $v0
    ctx->r2 = lo;
    // nop

    // nop

    // mult        $s0, $s0
    result = S64(S32(ctx->r16)) * S64(S32(ctx->r16)); lo = S32(result >> 0); hi = S32(result >> 32);
    // mflo        $v1
    ctx->r3 = lo;
    // jal         0x800746C4
    // addu        $a0, $v0, $v1
    ctx->r4 = ADD32(ctx->r2, ctx->r3);
    SquareRoot0(rdram, ctx);
    goto after_0;
    // addu        $a0, $v0, $v1
    ctx->r4 = ADD32(ctx->r2, ctx->r3);
    after_0:
    // addu        $a0, $s0, $zero
    ctx->r4 = ADD32(ctx->r16, 0);
    // addu        $s0, $v0, $zero
    ctx->r16 = ADD32(ctx->r2, 0);
    // jal         0x80076EA4
    // addu        $a1, $s1, $zero
    ctx->r5 = ADD32(ctx->r17, 0);
    ratan2(rdram, ctx);
    goto after_1;
    // addu        $a1, $s1, $zero
    ctx->r5 = ADD32(ctx->r17, 0);
    after_1:
    // sll         $v0, $v0, 16
    ctx->r2 = S32(ctx->r2) << 16;
    // lh          $a0, 0x20($s5)
    ctx->r4 = MEM_HS(0X20, ctx->r21);
    // sra         $v0, $v0, 16
    ctx->r2 = S32(ctx->r2) >> 16;
    // jal         0x800744C4
    // subu        $a0, $a0, $v0
    ctx->r4 = SUB32(ctx->r4, ctx->r2);
    rcos(rdram, ctx);
    goto after_2;
    // subu        $a0, $a0, $v0
    ctx->r4 = SUB32(ctx->r4, ctx->r2);
    after_2:
    // lh          $v1, 0x22($s5)
    ctx->r3 = MEM_HS(0X22, ctx->r21);
    // nop

    // mult        $v1, $v0
    result = S64(S32(ctx->r3)) * S64(S32(ctx->r2)); lo = S32(result >> 0); hi = S32(result >> 32);
    // mflo        $v0
    ctx->r2 = lo;
    // bgez        $v0, L_8004F658
    if (SIGNED(ctx->r2) >= 0) {
        // nop
    
        goto L_8004F658;
    }
    // nop

    // addiu       $v0, $v0, 0xFFF
    ctx->r2 = ADD32(ctx->r2, 0XFFF);
L_8004F658:
    // sra         $v0, $v0, 12
    ctx->r2 = S32(ctx->r2) >> 12;
    // lhu         $v1, 0x4($s6)
    ctx->r3 = MEM_HU(0X4, ctx->r22);
    // lhu         $a0, 0x1C($s5)
    ctx->r4 = MEM_HU(0X1C, ctx->r21);
    // subu        $v0, $s0, $v0
    ctx->r2 = SUB32(ctx->r16, ctx->r2);
    // addu        $v1, $v1, $a0
    ctx->r3 = ADD32(ctx->r3, ctx->r4);
    // slt         $v0, $v0, $v1
    ctx->r2 = SIGNED(ctx->r2) < SIGNED(ctx->r3) ? 1 : 0;
    // beq         $v0, $zero, L_8004F6D0
    if (ctx->r2 == 0) {
        // nop
    
        goto L_8004F6D0;
    }
    // nop

    // lw          $a1, 0x16($s2)
    ctx->r5 = MEM_W(0X16, ctx->r18);
    // lw          $a0, 0x8($s5)
    ctx->r4 = MEM_W(0X8, ctx->r21);
    // nop

    // slt         $v0, $a1, $a0
    ctx->r2 = SIGNED(ctx->r5) < SIGNED(ctx->r4) ? 1 : 0;
    // beq         $v0, $zero, L_8004F6B0
    if (ctx->r2 == 0) {
        // nop
    
        goto L_8004F6B0;
    }
    // nop

    // lhu         $v0, 0x1E($s5)
    ctx->r2 = MEM_HU(0X1E, ctx->r21);
    // nop

    // subu        $v0, $a0, $v0
    ctx->r2 = SUB32(ctx->r4, ctx->r2);
    // slt         $v0, $a1, $v0
    ctx->r2 = SIGNED(ctx->r5) < SIGNED(ctx->r2) ? 1 : 0;
    // beq         $v0, $zero, L_8004F6C8
    if (ctx->r2 == 0) {
        // addiu       $v0, $zero, -0x1
        ctx->r2 = ADD32(0, -0X1);
        goto L_8004F6C8;
    }
    // addiu       $v0, $zero, -0x1
    ctx->r2 = ADD32(0, -0X1);
    // j           L_8004F6E8
    // nop

    goto L_8004F6E8;
    // nop

L_8004F6B0:
    // lhu         $v1, 0x0($s2)
    ctx->r3 = MEM_HU(0X0, ctx->r18);
    // nop

    // subu        $v1, $a1, $v1
    ctx->r3 = SUB32(ctx->r5, ctx->r3);
    // slt         $v1, $a0, $v1
    ctx->r3 = SIGNED(ctx->r4) < SIGNED(ctx->r3) ? 1 : 0;
    // bne         $v1, $zero, L_8004F6E8
    if (ctx->r3 != 0) {
        // addiu       $v0, $zero, -0x1
        ctx->r2 = ADD32(0, -0X1);
        goto L_8004F6E8;
    }
    // addiu       $v0, $zero, -0x1
    ctx->r2 = ADD32(0, -0X1);
L_8004F6C8:
    // j           L_8004F6E8
    // addu        $v0, $s4, $zero
    ctx->r2 = ADD32(ctx->r20, 0);
    goto L_8004F6E8;
    // addu        $v0, $s4, $zero
    ctx->r2 = ADD32(ctx->r20, 0);
L_8004F6D0:
    // addiu       $s4, $s4, 0x1
    ctx->r20 = ADD32(ctx->r20, 0X1);
    // addiu       $s2, $s2, 0x58
    ctx->r18 = ADD32(ctx->r18, 0X58);
    // slti        $v0, $s4, 0xF0
    ctx->r2 = SIGNED(ctx->r20) < 0XF0 ? 1 : 0;
    // bne         $v0, $zero, L_8004F57C
    if (ctx->r2 != 0) {
        // addiu       $s3, $s3, 0x58
        ctx->r19 = ADD32(ctx->r19, 0X58);
        goto L_8004F57C;
    }
    // addiu       $s3, $s3, 0x58
    ctx->r19 = ADD32(ctx->r19, 0X58);
    // addiu       $v0, $zero, -0x1
    ctx->r2 = ADD32(0, -0X1);
L_8004F6E8:
    // lw          $ra, 0x30($sp)
    ctx->r31 = MEM_W(0X30, ctx->r29);
    // lw          $s7, 0x2C($sp)
    ctx->r23 = MEM_W(0X2C, ctx->r29);
    // lw          $s6, 0x28($sp)
    ctx->r22 = MEM_W(0X28, ctx->r29);
    // lw          $s5, 0x24($sp)
    ctx->r21 = MEM_W(0X24, ctx->r29);
    // lw          $s4, 0x20($sp)
    ctx->r20 = MEM_W(0X20, ctx->r29);
    // lw          $s3, 0x1C($sp)
    ctx->r19 = MEM_W(0X1C, ctx->r29);
    // lw          $s2, 0x18($sp)
    ctx->r18 = MEM_W(0X18, ctx->r29);
    // lw          $s1, 0x14($sp)
    ctx->r17 = MEM_W(0X14, ctx->r29);
    // lw          $s0, 0x10($sp)
    ctx->r16 = MEM_W(0X10, ctx->r29);
    // addiu       $sp, $sp, 0x38
    ctx->r29 = ADD32(ctx->r29, 0X38);
    // jr          $ra
    // nop

    return;
    // nop

;}
