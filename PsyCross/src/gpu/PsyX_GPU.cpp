...
//KF2 RAM
uint8_t* g_PsyX_Memory = nullptr;	


void PsyX_SetMemoryPointer(uint8_t* mem_ptr) {
	g_PsyX_Memory = mem_ptr;
}
...
//	printf("[DrawAllSplits] g_vertexIndex=%d g_splitIndex=%d\n", g_vertexIndex, g_splitIndex);
	...
	// Финализировать последний сплит перед рендером!
...
void ParsePrimitivesLinkedList(u_long* p, int singlePrimitive)
{
	...
	else
	{
		...
		while (true)
		{
			if (basePacket == (uintptr_t)-1 || basePacket == 0) {
				break;
			}

			// --- ФИКС АРХИТЕКТУРЫ: Читаем заголовок напрямую ---
			// Игнорируем кривые битовые поля MSVC!
			uint32_t header = *reinterpret_cast<uint32_t*>(basePacket);

			int tagLength = (header >> 24) & 0xFF;           // Старшие 8 бит - длина
			uint32_t next_ps1_addr = header & 0x00FFFFFF;    // Младшие 24 бита - адрес
			...
			if (tagLength > 0)
			{
				...
				while (currentPacket < endPacket)
				{
					...
					// Защита от бесконечного цикла, если FromSoft прислал длину 0
					if (primLength <= 0) primLength = 1;
			...
			if (next_ps1_addr == 0x00FFFFFF || next_ps1_addr == 0) {
				break; // Конец списка Ordering Table
			}

			if (!g_PsyX_Memory) {
				eprinterr("FATAL ERROR: g_PsyX_Memory is NULL!\n");
				break;
			}

			// Превращаем 24-битный PS1-адрес в реальный указатель памяти ПК
			basePacket = reinterpret_cast<uintptr_t>(&g_PsyX_Memory[next_ps1_addr & 0x1FFFFF]);
		}
	}
}
