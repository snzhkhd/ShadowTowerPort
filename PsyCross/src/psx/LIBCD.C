...
CdlFILE* CdSearchFile(CdlFILE* fp, char* name)
{
	...
	while (toc->tocEntryLength != 0)
	{
		...
		if (!strcmp(itemNameStr, pathPart))
		{
			...
			//CdIntToPos(toc->sectorPosition[0], &fp->pos);
#if _DEBUG
			eprintf("Found\n");
#endif
			return fp;
...
int CdControl(u_char com, u_char * param, u_char * result)
{
	...
	case CdlSeekL:
	case CdlSeekP:
		//  оманда SeekL использует последние установленные через Setloc координаты
		// ¬ Psy-X мы можем просто еще раз подтвердить позицию в файле
		// (g_cdCurrentSector должен быть установлен в CdlSetloc)
		eprintinfo("CdlSeek\n");
		vfseek(&g_imageFile, g_cdCurrentSector * g_cdSectorSize, SEEK_SET);
		break;
	case CdlPause:
		return 1;
